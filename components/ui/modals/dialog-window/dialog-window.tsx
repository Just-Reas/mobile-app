import { DialogWindowTheme } from "@/constants/theme";
...
export interface DialogWindowStyles {
  overlay?: ViewStyle;
  dialogWindowContainer?: ViewStyle;
  dialogWindowInner?: ViewStyle;
...
const DialogWindow: React.FC<DialogWindowProps> = ({
  ...
  const theme =
    colorScheme === "dark" ? DialogWindowTheme.dark : DialogWindowTheme.light;
  ...
  const mergedStyles = (
    Object.keys(theme) as Array<keyof DialogWindowStyles>
  ).reduce((acc, key) => {
  ...
  const iconRender = () => {
    switch (typeOfWindow) {
      ...
      default:
        return (
          <Svg width="48" height="48" viewBox="0 0 25 24" fill="none">
            <Path
  ...
  const renderContent = () => (
    <>
      ...
      <Animated.View
        style={[
          theme.dialogWindowContainer,
          mergedStyles.dialogWindowContainer,
          {
            opacity: opacityAnim,
      ...
      >
        <View style={[theme.dialogWindowInner, mergedStyles.dialogWindowInner]}>
          {onClose && (
            <TouchableOpacity
              style={[theme.closeButton, mergedStyles.closeButton]}
              onPress={onClose}
            >
              <Text
                style={{
          ...
          <View style={[theme.buttonBox, mergedStyles.buttonBox]}>
            {onCancel && (
              <TouchableOpacity
                style={[theme.cancelButton, mergedStyles.cancelButton]}
                onPress={onCancel}
              >
                <Text
                  style={[
                    theme.cancelButtonText,
                    mergedStyles.cancelButtonText,
                  ]}
                >
                  {cancelText}
                </Text>
            ...
            {onSuccess && (
              <TouchableOpacity
                style={[theme.successButton, mergedStyles.successButton]}
                onPress={onSuccess}
              >
                <Text
                  style={[
                    theme.successButtonText,
                    mergedStyles.successButtonText,
                  ]}
                >
                  {successText}
                </Text>
